	...
	if (status->base != -1)
	{
			...
			((rc = GET(status)) != EOF))
		{
			...
			else
			{
				...
				char * digitptr = (char*)nmemchr(_PDCLIB_digits, tolower(rc), status->base);
				if (rc != '.') {
					if (digitptr == NULL) {
						/* end of input item */
						UNGET(rc, status);
...
void TFLASHFunc(TFLASH *pTP)
{
    if(pTP->IN)
    {
        if((pTP->ET + pTP->PT1) > millis())
            pTP->Q = 1;  
        else if((pTP->ET + pTP->PT1) <= millis() && (pTP->ET + pTP->PT2) > millis())
            pTP->Q = 0;  
        else if((pTP->ET + pTP->PT2) < millis())
            pTP->ET = millis();
    }
    else
    {
