...
extern void SetThrottleCallback(uint8_t nDir, short nSpeed);

extern void WriteCVMainCallback(int nCV, unsigned short nNewVal);
extern void SetFunctionCallback(uint8_t nCnt, uint8_t Byte1, uint8_t Byte2);

extern void writeCVBitMainCallback(int nCV, uint8_t nBit, unsigned short nNewVal);
...
void RegisterList::setThrottle(char *s) volatile
{
    ...
    if(nsscanf(s, "%d %d %d %d", &nReg, &cab, &tSpeed, &tDirection)!=4)
    {
    ...
    // Check Cab Address
    if (cab != EEStore::eeStore->data.nCABAddr)
      return;
    
    if (cab > 127) // if long address
    	b[nB++] = highByte(cab) | 0xC0;      // convert train number into a two-byte address // b[0] = highbyte of CAB address
      
    b[nB++] = lowByte(cab);				 // b[1] or b[0] = low byte of CAB address
    b[nB++] = 0x3F;                      // 128-step speed control byte b[2], b[1] = 0x3F(63) 
    ...
    if (Client.connected())
    {
    	...
    	Client.print(nReg); Client.print(" ");
    	Client.print(tSpeed); Client.print(" ");
    	Client.print(tDirection);
    	Client.print(">");
    }
    speedTable[nReg]=tDirection==1?tSpeed:-tSpeed;

    SetThrottleCallback(tDirection, tSpeed);
...
void RegisterList::setFunction(char *s) volatile{
  ...
  nParams=nsscanf(s,"%d %d %d",&cab,&fByte,&eByte);
  ...
  if (cab != EEStore::eeStore->data.nCABAddr)
    return;
  SetFunctionCallback(nParams, fByte, eByte);
...
void RegisterList::setAccessory(char *s) volatile{
  ...
  if(nsscanf(s,"%d %d %d",&aAdd,&aNum,&activate)!=3)
    return;
...
void RegisterList::writeTextPacket(char *s) volatile
{
	...
	nBytes=nsscanf(s,"%d %x %x %x %x %x",&nReg,b,b+1,b+2,b+3,b+4)-1;
  
	if(nBytes<2 || nBytes>5){    // invalid valid packet
		if(Client.connected())
			Client.print("<mInvalid Packet>");
		return;
	}
...
void RegisterList::readCV(char *s) volatile{
	...
	if(nsscanf(s,"%d %d %d",&cv,&callBack,&callBackSub)!=3)          // cv = 1-1024
	return;    
...
void RegisterList::writeCVByte(char *s) volatile{
	...
	if(nsscanf(s,"%d %d %d %d",&cv,&bValue,&callBack,&callBackSub)!=4)          // cv = 1-1024
	return;    
	cv--;                              // actual CV addresses are cv-1 (0-1023)
...
void RegisterList::writeCVBit(char *s) volatile{
	...
	if(nsscanf(s,"%d %d %d %d %d",&cv,&bNum,&bValue,&callBack,&callBackSub)!=5)          // cv = 1-1024
	return;    
	cv--;                              // actual CV addresses are cv-1 (0-1023)
	bValue=bValue%2;
	bNum=bNum%8;
...
void RegisterList::writeCVByteMain(char *s) volatile{
	...
	if(nsscanf(s,"%d %d %d",&cab,&cv,&bValue)!=3)
		return;
...
void RegisterList::writeCVBitMain(char *s) volatile{
  ...
  if(nsscanf(s,"%d %d %d %d",&cab,&cv,&bNum,&bValue)!=4)
    return;
  if(cab != EEStore::eeStore->data.nCABAddr)
    return;

  writeCVBitMainCallback(cv, bNum, bValue);
...
void RegisterList::printPacket(int nReg, byte *b, int nBytes, int nRepeat) volatile {
	...
	for(int i = 0; i < nBytes; i++)
	{
		...
		{
			Client.print(" ");
			Client.print(b[i], HEX);
		}
	}
	if (Client.available())
	{
		Client.print(" / ");
		Client.print(nRepeat);
		Client.print(">");
	}
