...
///////////////////////////////////////////////////////////////////////////////
extern int DEFAULTCAB;
extern unsigned short nLongAddrFlag;
...
void EEStore::init(){
  ...
  if(strncmp(eeStore->data.id,EESTORE_ID,sizeof(EESTORE_ID))!=0){    // check to see that eeStore contains valid DCC++ ID
    ...
    eeStore->data.nShortCAB = DEFAULTCAB;
    eeStore->data.nLongCAB = DEFAULTCAB;
    eeStore->data.bLongAddrEnable = nLongAddrFlag;
    eeStore->data.nTurnouts = 0;
    eeStore->data.nSensors = 0;
    eeStore->data.nOutputs = 0;
    EEPROM.put(0,eeStore->data);    
  }
  if(eeStore->data.bLongAddrEnable)
  { // Long Address mode
      eeStore->data.nCABAddr = eeStore->data.nLongCAB;
  }
  else
  { // Short Address mode
      eeStore->data.nCABAddr = eeStore->data.nShortCAB;
  }
