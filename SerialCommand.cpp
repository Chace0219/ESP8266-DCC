  ...
  if (Client.connected())
	{
		if (Client.available())
		{
			...
			while (Client.available())
			{
				...
				//Serial.write(c);
				if (c == '<' || c == 0x0D)                    // start of new command
        {
            sprintf(commandString, "");
        }
				else if (c == '>')
...
{
  if(com[0] == ' ') // if first character is blank, it ignores.
    com = com + 1;
	switch(com[0])
	{
     ...
     break;
		...
		case 'c':     // <c>
		{
			...
			*/
			...
			{
				Client.print("<a");
				Client.print(int(mMonitor->current));
				Client.print(">");
			}
		}
		break;
 ...
 */
		...
		if (digitalRead(0) == LOW)      // could check either PROG or MAIN
		{
			if (Client.connected())
				Client.print("<p0>");
		}
		...
		for(int i = 1; i <= MAX_MAIN_REGISTERS; i++)
		{
			...
			if (Client.connected())
			{
				...
				if (mRegs->speedTable[i]>0) 
				{
					Client.print(mRegs->speedTable[i]);
					Client.print(" 1>");
				}
				else 
				{
 ...
 */
	  ...
	  {
		  Client.print("<f");
		  Client.print((int)&v - (__brkval == 0 ? (int)&__heap_start : (int)__brkval));
		  Client.print(">");
	  }*/
      break;
 ...
 */
		if (Client.connected())
		{
			...
			for (Register *p = mRegs->reg;p <= mRegs->maxLoadedReg;p++) {
				Client.print("M"); Client.print((int)(p - mRegs->reg)); Client.print(":\t");
				Client.print((int)p); Client.print("\t");
				Client.print((int)p->activePacket); Client.print("\t");
				Client.print(p->activePacket->nBits); Client.print("\t");
				for (int i = 0;i<10;i++) {
					Client.print(p->activePacket->buf[i], HEX); Client.print("\t");
				}
				Client.println("");
			}
			for (Register *p = pRegs->reg;p <= pRegs->maxLoadedReg;p++) {
				Client.print("P"); Client.print((int)(p - pRegs->reg)); Client.print(":\t");
				Client.print((int)p); Client.print("\t");
				Client.print((int)p->activePacket); Client.print("\t");
				Client.print(p->activePacket->nBits); Client.print("\t");
				for (int i = 0;i<10;i++) {
